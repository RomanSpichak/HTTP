...
/**
 * Fill the table and change cell of current row
 * @param {*} json 
 * @param {*} config 
 * @param {*} tbody 
 * @param {*} table 
...
function fillTable(json, config, tbody, table, parent) {
  for (const [key, value] of Object.entries(json.data)) {
    ...
    editBtn.innerHTML = 'Change row';
    editBtn.addEventListener('click', () => {
        showEditForm(config, key, value, row);
    });
...
/**
 * Create div to form and fill changes row in form
 * @param {*} config 
 * @param {*} id 
 * @param {*} currentData 
 */
function showEditForm(config, id, currentData, row) {
    ...
    config.columns.map(column => {
        ...
        inputs.forEach(inputConfig => {
            ...
            if (inputConfig.type === 'select') {
                ...
                if (String(opt).trim() === String(value).trim()) {
    ...
    form.addEventListener('submit', event => {
        ...
        let hasError = false;
        form.querySelectorAll('input, select').forEach(input => {
            hasError = checkError(formData, input, hasError, updateData);
        });
        if (hasError) return;
